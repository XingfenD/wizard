...
<script>
    $(function () {
        window.ui = SwaggerUIBundle({
            ...
            url: "{!! wzRoute('project:doc:history:json', ['code' => $code ?? '', 'id' => $project->id, 'page_external_id' => $pageItem->external_id, 'history_id' => $history->id, 'only_body' => 1, 'ts' => microtime(true)]) !!}"
            @else
            url: "{!! wzRoute('project:doc:json', ['keyword' => $keyword ?? '', 'code' => $code ?? '','id' => $project->id, 'page_external_id' => $pageItem->external_id, 'only_body' => 1, 'ts' => microtime(true)])  !!}"
            @endif
        });
